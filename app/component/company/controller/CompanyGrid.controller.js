sap.ui.define([
    ...
    'use strict';
    let totalNumber;
    return Controller.extend("project2.controller.CompanyGrid",{
        ...
        onNavToHome: function (oEvent) {
            ...
            this.getOwnerComponent().getRouter().navTo("Request_Home"); //컴포넌트나 메니페스트에 저장된 roter정보를 가져옴 그중에서도 viewname:CreatOrder에 기반한 정보를 가져오고 해당 정보를 가지고 페이지를 전환함.
            


        },
        onInit: async function () {           
            const myRoute = this.getOwnerComponent().getRouter().getRoute("CompanyGrid");
        ...
        onNavToDetail: function (oEvent) {
            ...
            var SelectedNum = oEvent.getParameters().row.mAggregations.cells[0].mProperties.text;
            this.getOwnerComponent().getRouter().navTo("CompanyDetail", { num: SelectedNum, table: "GridTable" }); //컴포넌트나 메니페스트에 저장된 roter정보를 가져옴 그중에서도 viewname:CreatOrder에 기반한 정보를 가져오고 해당 정보를 가지고 페이지를 전환함.



        },
        onCompanyMTable: function(){
        ...
        onSearch: function () {
            ...
            if (Date) {
                aFilter.push(new Filter("comdate", FilterOperator.Contains, Date))
            }
            if (Status) {
                aFilter.push(new Filter("comstate", FilterOperator.Contains, Status))
            }
            let oTable = this.byId("CompanyGridTable").getBinding("rows");
        ...
        onConfirmSortCompanyDialog: function (oEvent) { //팝업창에서 확인을 눌렀을때의 동작
            ...
            let oBinding = this.byId("CompanyGridTable").getBinding("rows"); 
            oBinding.sort(aSorters);   //위에서 만든 Sorter를 실제로 적용하겠다는 의미
        },

        onCompanyAdd: function () {
            let CompanyAdd = this.getView().getModel("CompanyModel").oData;
            let CompanyAddIndex= CompanyAdd.length;
            let CreateNum = parseInt(CompanyAdd[CompanyAddIndex - 1].comcode) + 1
            this.getOwnerComponent().getRouter().navTo("CompanyAdd",{num: CreateNum})
        },
        onDeleteCompany : async function(){
            console.log(this.byId("CompanyGridTable"))
            var aIndices = this.byId("CompanyGridTable").getSelectedIndices();
            var sMsg;
